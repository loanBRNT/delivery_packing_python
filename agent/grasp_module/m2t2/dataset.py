...
def load_rgb_xyz(
    data_dir, robot_prob, world_coord, jitter_scale, grid_res, surface_range=0, depth=None, meta_data=None, seg=None, rgb=None
):
    ...
    if rgb is None:
        rgb = Image.open(f'{data_dir}/rgb.png')
    print(rgb)
    ...
    normalized_depth = (depth / np.max(depth) * 255).astype(np.uint8)

    # Save as a grayscale PNG
    Image.fromarray(normalized_depth).save("depth_mani.png")
    Image.fromarray(np.uint8(seg)).save("seg_mani.png")

    
    label_map = meta_data['label_map']

    if torch.rand(()) > robot_prob: #je sais pas trop a quoi ça sert robot_prob = 1
